...
def notify_mentions(content, sender, post_id=None):
    """
    ...
    for user in users:
        Notification.objects.create(
            sender=sender,
            recipient=user,
            notification_type='comment',  # puoi usare un tipo separato se vuoi
            post_id=post_id,
            message=f"@{sender.username} ti ha menzionato in un {'commento' if post_id else 'post'}."
        )
